...
static void gingado() {
  for (t = 0; t < 10.0; t += TIME_STEP / 1000.0) {
    ...
    {
      wb_motor_set_position(pistons[i], AMPL * -cos(phase));
    }
...
void deley(int valor){
  int count =  valor *10000;
...
void myFunction(double FREQ,double ampl,double phase)
{
    wb_motor_set_position(pistons[1], ampl * sin(phase));
    wb_motor_set_position(pistons[2], ampl * sin(phase));
    wb_motor_set_position(pistons[3], ampl * sin(phase));
    wb_motor_set_position(pistons[4], ampl * sin(phase));
    wb_motor_set_position(pistons[5], ampl * sin(phase));
    wb_motor_set_position(pistons[0], ampl * sin(phase));
